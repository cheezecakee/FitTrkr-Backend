...
func (r *DBPlanRepo) GetByID(ctx context.Context, id uint) (*m.Plan, error) {
	plan := &m.Plan{}
	row := r.tx.DB().QueryRowContext(ctx, getPlanByID, id)
	err := row.Scan(
		&plan.ID,
		&plan.UserID,
		&plan.Name,
...
func (r *DBPlanRepo) GetByUserID(ctx context.Context, userID uuid.UUID) ([]*m.Plan, error) {
	rows, err := r.tx.DB().QueryContext(ctx, getPlanByUserID, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *DBPlanRepo) List(ctx context.Context, offset, limit int) ([]*m.Plan, error) {
	rows, err := r.tx.DB().QueryContext(ctx, listPlan, offset, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
